...
func (i *internalPKI) GetControllerTLSConfig() (config *tls.Config, err error) {
	config = &tls.Config{}
	tlsKeys := &corev1.Secret{}
	err = i.client.Get(context.TODO(),
		types.NamespacedName{
			Namespace: i.cluster.Namespace,
			Name:      fmt.Sprintf(pkicommon.BrokerControllerTemplate, i.cluster.Name),
		},
		tlsKeys,
	)
	if err != nil {
