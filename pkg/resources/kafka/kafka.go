...
// certsPEM is for storing the PEM certs for PEM format based listener SSL setting
type certsPEM map[string]string
...
func (r *Reconciler) getClientPasswordKeyUserAndFormat() (string, string, bool, error) {
	var clientPass, CN string
	var jksFormatCert bool
	if r.KafkaCluster.Spec.IsClientSSLSecretPresent() {
		...
		if err := r.Client.Get(context.TODO(), clientNamespacedName, clientSecret); err != nil {
			if apierrors.IsNotFound(err) && r.KafkaCluster.Spec.GetClientSSLCertSecretName() == "" {
				return "", "", false, errorfactory.New(errorfactory.ResourceNotReady{}, err, "client secret not ready")
			}
			return "", "", false, errors.WrapIfWithDetails(err, "failed to get client secret")
		}
		pem, jks, err := util.CheckSSLCertSecret(clientSecret)
		if !pem && !jks {
			if r.KafkaCluster.Spec.GetClientSSLCertSecretName() != "" {
				return "", "", false, err
			}
			return "", "", false, errorfactory.New(errorfactory.ResourceNotReady{}, errors.Errorf("SSL JKS certificate has not generated properly yet into secret: %s", clientSecret.Name), "checking secret data fields")
		}
		var cert *x509.Certificate
		if jks {
			tlsCert, err := util.ParseTLSCertFromKeyStore(clientSecret.Data[v1alpha1.TLSJKSKeyStore], clientSecret.Data[v1alpha1.PasswordKey])
			if err != nil {
				return "", "", false, errors.WrapIfWithDetails(err, fmt.Sprintf("failed to decode certificate, secretName: %s", clientSecret.Name))
			}
			cert = tlsCert.Leaf
			jksFormatCert = true
		} else if pem {
			keyPair, err := util.SplitKeystorePEM(clientSecret.Data[v1alpha1.TLSPEMKeyStore])
			if err != nil {
				return "", "", false, errors.WrapIfWithDetails(err, fmt.Sprintf("failed to parse certificate %s, secretName: %s", v1alpha1.TLSPEMKeyStore, clientSecret.Name))
			}
			cert, err = certutil.DecodeCertificate(keyPair[1])
			if err != nil {
				return "", "", false, errors.WrapIfWithDetails(err, fmt.Sprintf("failed to decode certificate, secretName: %s", clientSecret.Name))
			}
		}
	...
	return clientPass, CN, jksFormatCert, nil
}
...
func getListenerSSLCertSecret(client client.Reader, commonSpec v1beta1.CommonListenerSpec, clusterName string, clusterNamespace string) (*corev1.Secret, error) {
	...
	pem, jks, err := util.CheckSSLCertSecret(serverSecret)
	if !pem && !jks {
		if commonSpec.GetServerSSLCertSecretName() != "" {
			return nil, err
		}
...
func (r *Reconciler) getServerPasswordKeysUsersAndFormat() (map[string]string, []string, map[string]bool, error) {
	// pair contains the listenerName and the corresponding  JKS cert password
	pair := make(map[string]string)
	jksFormatCert := make(map[string]bool)
	var CNList []string
	...
	for _, iListener := range r.KafkaCluster.Spec.ListenersConfig.InternalListeners {
		if iListener.Type == v1beta1.SecurityProtocolSSL {
			...
			// Checking certificates availability
			pem, jks, err := util.CheckSSLCertSecret(serverSecret)
			if !pem && !jks {
				return nil, nil, nil, errors.WrapIfWithDetails(err, "could not get configuration data for listener", "name", iListener.Name)
			}
			...
			if iListener.UsedForControllerCommunication || iListener.UsedForInnerBrokerCommunication {
				var cert *x509.Certificate
				if jks {
					tlsCert, err := util.ParseTLSCertFromKeyStore(serverSecret.Data[v1alpha1.TLSJKSKeyStore], serverSecret.Data[v1alpha1.PasswordKey])
					if err != nil {
						return nil, nil, nil, errors.WrapIfWithDetails(err, fmt.Sprintf("failed to decode certificate, secretName: %s", serverSecret.Name))
					}
					cert = tlsCert.Leaf
				} else if pem {
					keyPair, err := util.SplitKeystorePEM(serverSecret.Data[v1alpha1.TLSPEMKeyStore])
					if err != nil {
						return nil, nil, nil, errors.WrapIfWithDetails(err, fmt.Sprintf("failed to parse certificate %s, secretName: %s", v1alpha1.TLSPEMKeyStore, serverSecret.Name))
					}
					cert, err = certutil.DecodeCertificate(keyPair[1])
					if err != nil {
						return nil, nil, nil, errors.WrapIfWithDetails(err, fmt.Sprintf("failed to decode certificate, secretName: %s", serverSecret.Name))
			...
			if jks {
				jksFormatCert[iListener.Name] = true
			}
		}
	}
	...
	for _, eListener := range r.KafkaCluster.Spec.ListenersConfig.ExternalListeners {
		if eListener.Type == v1beta1.SecurityProtocolSSL {
			...
			pem, jks, err := util.CheckSSLCertSecret(serverSecret)
			if !pem && !jks {
				return nil, nil, nil, errors.WrapIfWithDetails(err, "could not get configuration data for listener", "name", eListener.Name)
			}
			...
			// Collecting the PEM based server certificates if JKS is not available
			if jks {
				jksFormatCert[eListener.Name] = true
			}
		}
	}
	return pair, CNList, jksFormatCert, nil
}
func (r *Reconciler) getPasswordKeysSuperUsersAndFormat() (clientPass string, serverPasses map[string]string, superUsers []string, serverJKSFormatCert map[string]bool, clientJKSFormatCert bool, err error) {
	serverPasses, superUsers, serverJKSFormatCert, err = r.getServerPasswordKeysUsersAndFormat()
	if err != nil {
		return "", nil, nil, nil, false, err
	}
	clientPass, superUser, clientJKSFormatCert, err := r.getClientPasswordKeyUserAndFormat()
	if err != nil {
		return "", nil, nil, nil, false, err
	}
	if superUser != "" {
	...
	return clientPass, serverPasses, superUsers, serverJKSFormatCert, clientJKSFormatCert, nil
}
