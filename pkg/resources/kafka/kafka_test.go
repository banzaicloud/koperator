...
func TestGetServerPasswordKeysAndUsers_PasswordSecretNotFilled(t *testing.T) {
	t.Parallel()
	testCases := []struct {
		testName          string
		listenerType      string
	...
	}{
		{
			...
			listenerType: "SSL",
			secrets: map[string]corev1.Secret{"internal": {
	...
	for _, test := range testCases {
		...
		t.Run(test.testName, func(t *testing.T) {
			...
			assert.NotNil(t, err, "Should be an error because there are missing secret data fields")
			assert.Equal(t, len(serverPasswords), 0, "serverPasswords should be empty")
		})
	}
}

func TestGetServerPasswordKeysAndUsers(t *testing.T) {
	t.Parallel()
	...
	for _, test := range testCases {
		...
		t.Run(test.testName, func(t *testing.T) {
			...
			expectedServerPasses := make(map[string]string)
			serverSecret := &corev1.Secret{}
			// expectedGetCount is a counter for the expected Client.Get calls
			// Client.Get shouldnt be called more than once for listeners which are used autogenerated SSL
			expectedGetCount := 0
			first := true
			for _, listener := range test.internalListeners {
			...
			serverPasswords, _, err := r.getServerPasswordKeysAndUsers()
			if test.testName != "nocertificate" {
				assert.Nil(t, err, err)
				assert.NotEqual(t, len(serverPasswords), 0, "serverPasswords should'nt be empty")
